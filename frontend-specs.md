...
**Estado:** Frontend y backend implementados — listos para integración end-to-end
...
## 9. Notas de integración frontend ↔ backend
...
Con `VITE_MOCK=true`, `useSession` inyecta un usuario admin hardcoded (`dev@empresa.com`) y omite `GET /api/auth/me`. Todas las demás llamadas a la API siguen apuntando al backend real vía `VITE_API_URL`.
...
### 9.3 Pendiente para integración completa
...
- Desactivar `VITE_MOCK=true` una vez que el backend de auth esté operativo
...
| 1.2 | 2026-05-05 | Backend implementado: todos los servicios y rutas; CORS configurado; `VITE_API_URL` añadido; `mini-jira/plan.md` (Kanban) marcado como completado |
